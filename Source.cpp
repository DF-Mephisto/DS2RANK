...
void SaveFile(vector<Enemy>* statistic, vector<Enemy>* statisticWorld, wchar_t* path)
{
	...
	if (StatFile.is_open()) {
		wstring globalSize = to_wstring(statistic->size() + statisticWorld->size()) + L'\t';
		encryptStr(&globalSize);
		StatFile << globalSize;
...
void ReadFileToStr(vector<Enemy>* bloodlist, vector<Enemy>* worldlist, wstring* str)
{
	...
	for (int i = 0; i < globalSize; i++) {
		...
		//STATS
		stream >> temps;
		p.SetStrPar(pNAME, temps);

		stream >> tempi;
		p.SetIntPar(pLVL, tempi);

		stream >> temps;
		p.SetStrPar(pRES, temps);

		stream >> tempi;
		p.SetIntPar(pHEALTH, tempi);

		stream >> tempi;
		p.SetIntPar(pSTM, tempi);

		stream >> tempi;
		p.SetIntPar(pVIG, tempi);

		stream >> tempi;
		p.SetIntPar(pEND, tempi);

		stream >> tempi;
		p.SetIntPar(pVIT, tempi);

		stream >> tempi;
		p.SetIntPar(pATN, tempi);

		stream >> tempi;
		p.SetIntPar(pSTR, tempi);

		stream >> tempi;
		p.SetIntPar(pDEX, tempi);

		stream >> tempi;
		p.SetIntPar(pADP, tempi);

		stream >> tempi;
		p.SetIntPar(pINT, tempi);

		stream >> tempi;
		p.SetIntPar(pFTH, tempi);


		//WEAPONS
		stream >> tempi;
		p.SetIntPar(pCOUNTS, tempi, 0);
		FillEquipVector(&p, &stream, pR1, p.GetIntPar(pCOUNTS, 0));

		stream >> tempi;
		p.SetIntPar(pCOUNTS, tempi, 1);
		FillEquipVector(&p, &stream, pR2, p.GetIntPar(pCOUNTS, 1));

		stream >> tempi;
		p.SetIntPar(pCOUNTS, tempi, 2);
		FillEquipVector(&p, &stream, pR3, p.GetIntPar(pCOUNTS, 2));

		stream >> tempi;
		p.SetIntPar(pCOUNTS, tempi, 3);
		FillEquipVector(&p, &stream, pL1, p.GetIntPar(pCOUNTS, 3));

		stream >> tempi;
		p.SetIntPar(pCOUNTS, tempi, 4);
		FillEquipVector(&p, &stream, pL2, p.GetIntPar(pCOUNTS, 4));

		stream >> tempi;
		p.SetIntPar(pCOUNTS, tempi, 5);
		...
		{
			stream >> temp;
			p.SetDwordPar(pCONS, temp, j);
		}

		//DATE AND MISC
		stream >> temps;
		p.SetStrPar(pDATE, temps);

		stream >> tempi;
		p.SetIntPar(pHOST, tempi);

		stream >> tempi;
		p.SetIntPar(pRANK, tempi);
